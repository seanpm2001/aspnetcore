...
internal static class HttpContextDebugFormatter
{
    ...
    {
        var text = $"{request.Method} {request.Scheme}://{request.Host.Value}{request.PathBase.Value}{request.Path.Value}{request.QueryString.Value} {request.Protocol}";
    ...
    {
        return $"Request = {RequestToString(context.Request)}, Response = {ResponseToString(context.Response, reasonPhrase)}";
    }
}
